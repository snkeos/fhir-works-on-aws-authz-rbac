...
const testRuns = [
    ...
    },
    {
        type: 'scopes',
        practitionerAccessToken:
            'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJmYWtlIiwic2NvcGUiOiJmaGlyd29ya3MvcHJhY3RpdGlvbmVyIiwibmFtZSI6Im5vdCByZWFsIiwiaWF0IjoxNTE2MjM5MDIyfQ.3GWbGhlaX3ZHemJ3Kyh7J0BHDIbnkEve43s8FzQ8A3c',
        practitionerDecoded: {
            sub: 'fake',
            scope: 'fhirworks/practitioner',
            name: 'not real',
            iat: 1516239022,
        },
        nonPractAndAuditorAccessToken:
            'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiJmYWtlIiwic2NvcGUiOiJmaGlyd29ya3MvYXVkaXRvciBmaGlyd29ya3Mvbm9uLXByYWN0aXRpb25lciIsIm5hbWUiOiJub3QgcmVhbCIsImlhdCI6MTUxNjIzOTAyMn0.-jXa08Aw4L0sjSs0KAkT-yhqQUpj7FjOZsaH4aLTov0',
        nonPractAndAuditorDecoded: {
            sub: 'fake',
            scope: 'fhirworks/auditor fhirworks/non-practitioner',
            name: 'not real',
            iat: 1516239022,
        },
    },
...
testRuns.forEach(run => {
    const { practitionerAccessToken } = run;
    const { practitionerDecoded } = run;
    const { nonPractAndAuditorAccessToken } = run;
    const { nonPractAndAuditorDecoded } = run;
    ...
    describe(run.type, () => {
        describe('verifyAccessToken', () => {
            ...
            test('GET Patient with non-practitioner/auditor; expected: pass', async () => {
                ...
                ).resolves.toEqual(nonPractAndAuditorDecoded);
            });
        ...
        describe('isBundleRequestAuthorized', () => {
            ...
            test('create & read direct patient in bundle with nonPractAndAuditor; expected: UnauthorizedError', async () => {
                await expect(
                    authZHandler.isBundleRequestAuthorized({
                        userIdentity: nonPractAndAuditorDecoded,
                        requests: [
                            { operation: 'read', id: 'id', resource: 'Patient/id', resourceType: 'Patient' },
                            { operation: 'create', id: 'id', resource: { active: true }, resourceType: 'Patient' },
                        ],
